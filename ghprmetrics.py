...
Options:
  ...
  --maxCount=<maxCount>           Maximum number of PRs to traverse (gather data). Limits the output [default: 4096]
...
def extract_gh_metrics(token, enterprise, repo_regex, state, only_merged, max_count, max_days, creator_regex):
    ...
    for repo in gh.get_user().get_repos():
        if re.search(repo_regex, repo.name):
            for pull in repo.get_pulls(state=state):
                ...
                total_time_reviewing = wait_time_for_last_review - wait_time_for_first_review
                yield [repo.name, pull.id, title, pull.state, creation_date, pull.user.login, pull.last_modified,
                       ...
                       int(total_time_reviewing.total_seconds() / 60)]
...
def main():
    ...
    csv_writer.writerow(["Repository", "PR id", "PR title", "PR state", "PR Creation Date", "PR Creator", "PR Last Modified", "PR is merged",
                         ...
                         "Count Review Requests", "Count Reviews", "Minutes to First Review", "Minutes to Last Review", "Minutes Reviewing"])
    for pr_data in  \
            extract_gh_metrics(arguments["--token"], arguments["--enterprise"],
                               arguments["--repos"], arguments["--state"], arguments["--merged"],
                               int(arguments["--maxCount"]), int(arguments["--maxDays"]), arguments["--creator"]):
